...
  // Mock AI response function since we can't use backend API
  const getMockAIResponse = async (message: string) => {
    await new Promise(resolve => setTimeout(resolve, 1500));
    return `[${selectedModel.name}] This is a simulated AI response to: "${message}".`;
  };
...
    if (!user) return;
    try {
      const docRef = doc(db, "users", user.uid, "features", "ai_usage");
...
    if (!user) return;
    try {
      // 1 message character ≈ 1 byte, convert bytes to GB (1GB = 1,073,741,824 bytes)
...
    try {
      if (!user) return;
      await deleteDoc(doc(db, "users", user.uid, "chatHistories", chatId));
...
    try {
      if (!user) return;
      const chatsCollection = collection(db, "users", user.uid, "chatHistories");
...
    try {
      if (!user) return;
      const chatDoc = await getDoc(doc(db, "users", user.uid, "chatHistories", id));
      if (chatDoc.exists()) {
...
      if (currentChatId) {
        await updateDoc(doc(db, "users", user.uid, "chatHistories", currentChatId), {
          messages: updatedMessages,
          updatedAt: Date.now(),
        });
      } else {
        const newChatDoc = await addDoc(collection(db, "users", user.uid, "chatHistories"), {
          title: chatTitle,
          messages: updatedMessages,
          createdAt: Date.now(),
          updatedAt: Date.now(),
...
      if (i === index) {
        // Toggle if clicking same feedback, or set new
...
    try {
      // Mock API call
      const aiResponseText = await getMockAIResponse(userMessage);
      
