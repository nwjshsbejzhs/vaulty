...
/**
 * VARNOSTNI FALLBACK: Ker okolje ne more razrešiti uvoza "@/contexts/premium-context", 
 * smo definirali lokalno funkcijo usePremium. V svojem projektu lahko to zamenjaš 
 * z originalnim importom, če pot deluje pravilno.
 */
const usePremium = () => {
  return { tier: "free", hasAccess: false };
};

// Konfiguracija za Gemini AI
const apiKey = "import { useState, useEffect, useRef } from "react";
import { useLocation, useRoute } from "wouter";
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar";
import { ArrowLeft, Send, Mic, Heart, Info, Reply, MoreVertical, Trash2, Camera, Loader2, Bot, Lock, X } from "lucide-react";
import { toast } from "sonner";
...
export default function CompanionChat() {
  ...
  const getAIResponseReal = async (userMessage: string, history: any[], companionData: any) => {
    ...
    const systemInstruction = `
      ...
      Language Rules:
      - Speak in the primary language of ${companionData.nationality}.
      - Use grammar and pronouns appropriate for your gender (${companionData.gender || 'female'}).

      Style Rules (CRITICAL):
      ${isCasual ? "- WRITE ONLY IN LOWERCASE. NO CAPITAL LETTERS AT ALL." : "- Use standard professional grammar."}
  ...
  const handleSend = async () => {
    ...
    // 100% Real typing time simulation based on response length
  ...
  return (
    <div className={`flex flex-col h-[100dvh] ${currentTheme?.chatBg} text-white relative overflow-hidden`}>
      {/* Header */}
      <header className="flex items-center justify-between p-4 bg-black/80 backdrop-blur-md sticky top-0 z-30 border-b border-white/10 flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-green-500 flex items-center gap-1 font-bold uppercase tracking-widest">
                  ...
                  <div className="p-4 space-y-4">
                    ...
                    {menuItem === "credits" && (
                      <div className="space-y-4">
                        <p className="font-bold text-sm tracking-tight">Usage</p>
                        <div className="bg-zinc-800 p-3 rounded-xl border border-white/5">
                          ...
                          <div className="w-full bg-white/10 h-1.5 rounded-full overflow-hidden">
                             <div className="h-full bg-blue-500 transition-all duration-500" style={{ width: `${(dailyUsage / dailyLimit) * 100}%` }} />
                          </div>
                        </div>
                        <div className="text-[10px] text-zinc-500 text-center font-bold uppercase">LIFETIME: {getTotalCredits()}</div>
                      </div>
                    )}
                    {menuItem === "theme" && (
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 flex flex-col scroll-smooth" ref={scrollRef}>
        {messages.length === 0 && (
          <div className="flex-1 flex flex-col items-center justify-center text-center opacity-20">
            ...
            <p className="text-sm font-bold uppercase tracking-[0.2em]">Connection Secured</p>
          </div>
        )}
          ...
          <div key={msg.id} className="flex flex-col">
            ...
            <div className={`flex ${msg.sender === "user" ? "justify-end" : "justify-start"} group relative items-end gap-2`}>
              ...
              <div className="relative max-w-[85%]">
                <div
                  // POSODOBLJENO: Zdaj deluje na preprost klik za mobilno odzivnost
                  onClick={() => setActiveReactionMessage(activeReactionMessage === msg.id ? null : msg.id)}
                  className={`px-4 py-2.5 rounded-2xl relative text-[15px] leading-relaxed shadow-xl cursor-pointer active:scale-[0.98] transition-all ${
                    msg.sender === "user"
                ...
                <AnimatePresence>
                  {activeReactionMessage === msg.id && (
                    <motion.div
                      ...
                      className="absolute bottom-full left-0 bg-zinc-900/90 backdrop-blur-xl border border-white/10 rounded-full p-1.5 flex gap-1.5 z-50 shadow-2xl"
                    >
                      {REACTIONS.map((emoji) => (
      ...
      {/* Input Area - FIX: flex-shrink-0 in h-[100dvh] zagotavljata pravilno pozicijo */}
      <footer className="shrink-0 p-4 bg-black/90 backdrop-blur-xl border-t border-white/10 z-40 pb-safe pb-[max(2rem,env(safe-area-inset-bottom))]">
        <AnimatePresence>
          {replyingTo && (
            <motion.div 
            ...
            >
              <div className="min-w-0 pr-4">
                <p className="text-[10px] font-black text-blue-400 uppercase tracking-widest mb-0.5">Replying to {replyingTo.sender === 'user' ? 'You' : companion.name}</p>
        ...
        <div className={`flex items-center gap-3 rounded-[2rem] px-3.5 py-2.5 border border-white/10 shadow-2xl transition-all focus-within:border-white/30 ${currentTheme?.inputBg}`}>
          ...
          <button 
            ...
            className={`flex-shrink-0 w-10 h-10 flex items-center justify-center transition-all active:scale-90 rounded-2xl ${
              input.trim() && !isTyping ? currentTheme?.buttonColor : "text-zinc-800 opacity-20"
            }`}
          >
        ...
        {showDeleteConfirm && (
          ...
          >
            ...
            >
              ...
              <p className="text-xs text-zinc-500 mb-10 leading-relaxed font-medium">All history with {companion.name} will be removed.</p>
              <div className="flex flex-col gap-3">
                <button 
                  ...
                  className="w-full py-4 bg-red-600 hover:bg-red-700 rounded-2xl text-xs text-white font-black uppercase tracking-widest transition-all active:scale-95 shadow-lg shadow-red-600/20"
                >
                  Confirm Delete
                </button>
                <button onClick={() => setShowDeleteConfirm(false)} className="w-full py-4 bg-white/5 hover:bg-white/10 rounded-2xl text-xs font-black uppercase tracking-widest transition-all">Cancel</button>
              </div>
            </motion.div>
          </motion.div>
        )}
